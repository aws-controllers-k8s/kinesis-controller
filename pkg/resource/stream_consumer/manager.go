...
// +kubebuilder:rbac:groups=kinesis.services.k8s.aws,resources=streamconsumers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kinesis.services.k8s.aws,resources=streamconsumers/status,verbs=get;update;patch
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.ConsumerStatus == nil {
		return false, nil
	}
	consumerStatusCandidates := []string{"ACTIVE"}
	if !ackutil.InStrings(*r.ko.Status.ConsumerStatus, consumerStatusCandidates) {
		return false, nil
	}
