...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.ConsumerName = &identifier.NameOrID

	f0, f0ok := identifier.AdditionalKeys["consumerARN"]
	if f0ok {
		r.ko.Status.ConsumerARN = aws.String(f0)
	}
	f2, f2ok := identifier.AdditionalKeys["streamARN"]
	if f2ok {
		r.ko.Spec.StreamARN = aws.String(f2)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["consumerName"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: consumerName"))
	}
	r.ko.Spec.ConsumerName = &primaryKey

	f0, f0ok := fields["consumerARN"]
	if f0ok {
		r.ko.Status.ConsumerARN = aws.String(f0)
	}
	f2, f2ok := fields["streamARN"]
	if f2ok {
		r.ko.Spec.StreamARN = aws.String(f2)
	}
